...
###### IAN LEWIS, JUNE 16
=begin
On line 1, the local variable `n` is assigned to the return value of invoking the `Integer#times` method on the integer `101` and passing in a block. 

The block itself iterates 101 times, and on each iteration a number from 0 up to 100 is passed in to the block in turn. The block variable `n` is assigned to that value on line 1, so on the first iteration it will be assigned to `0`, on the next it will be assigned to `1`, etc. Within the block, block variable `n` is then reassigned to the value `11` on line 2.
